...
PDF FAISS retrieval with optional product-scoped filtering.
...
def search(query: str, top_k: int = 3, matched_product: Optional[str] = None) -> list[dict]:
    ...
    if len(candidates) <= top_k:
        results = [m for _, m in candidates]
        print(f"[pdf_search] Filtered chunks={len(candidates)} | Returned chunks={len(results)}")
        return results
    ...
    results  = [candidates[r][1] for r in top_rows]
